...
The grammar permits member filters without a refset operator, but the logical model describes filters on memberOf results. The engine returns ParseErrorKind::Semantic because the operator-free meaning remains unresolved. [Upstream issue 10](https://github.com/IHTSDO/snomed-expression-constraint-language/issues/10) asks about this discrepancy. The error does not close the conformance item; see [open grammar forms](conformance.md#open-grammar-forms).

Predicates are typed by the stored column. Every combination the grammar admits but the column cannot hold, such as `mapGroup = "20260826"`, `active = #1`, `effectiveTime = true`, `id = 300001` or `mapTarget = ""`, fails with `TypeMismatch` before any row is read, including when an earlier predicate already excludes every row. Quoted text that is not a date is a string predicate; without the `unicode` feature it fails with `Unsupported` before the column type is checked. Member UUIDs match as text, so `id = wild:"0101*"` and a full UUID both select rows.
...
`memberOf` and `^R` operate on concept-referencing rows. Typed tables retain those rows, so non-concept `component` results come from other projected fields. New imports record `concept_refsets` and `non_concept_refsets` using RF2 descriptors and rows of every status. A selection containing only known non-concept refsets returns `Semantic`; mixed and wildcard selections retain concept members. Inactive concept members remain queryable, including those in REFERS TO. Older manifests lack this classification and retain their previous empty-result behaviour for non-concept-only refsets. Reimport to obtain the new diagnostics.
