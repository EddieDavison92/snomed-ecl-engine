...
Term matching remains required and currently returns an explicit unsupported error. The stored text supplies the data for `match:` and `wild:` queries, including the requested `match:"gas" wild:"*itis"` example. Implement locale-aware matching against the [ECL collation guidance](https://docs.snomed.org/snomed-ct-specifications/snomed-ct-expression-constraint-language/design/5-syntax-specification#character-collation-for-term-filters), then measure candidate indexes and compression. Simply removing every accent would lose the distinction between marked and unmarked query letters.
