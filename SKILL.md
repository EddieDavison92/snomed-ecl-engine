...
Inspect `supplements` in the combined manifest. Batch responses include supplement archive checksums; the edition URI still identifies the base. Exact module versions are not yet fully resolved. Supplementary descriptions and their language memberships are preserved when the base has a description index. Other maps and typed fields are outside this command's current scope. Read [refsets](docs/refsets.md) before importing a different extension format.
...
Description queries load `descriptions.bin` on demand. Reimport older stores to build it. The first such query includes file validation and loading; `store.descriptions.get()` can preload it. Term matching is still pending. See [description filters](docs/descriptions.md) for supported predicates and memory measurements.
