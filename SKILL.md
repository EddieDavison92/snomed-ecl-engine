...
Description queries load `descriptions.bin` on demand. The first such query includes file validation and loading; `store.descriptions.get()` can preload it. Term matching needs `--features unicode`. See [description filters](docs/ecl-support.md) for supported predicates and memory measurements.

Member filters and projections load the needed `members/<refset>.bin` files. Typed projections can return distinct scalar values or rows instead of concept codes: batch responses then have `result_type: "values"` with `values`, or `result_type: "rows"` with `rows`. Preserve these types; do not interpret map targets or numbers as SNOMED IDs. `--count` counts distinct values or rows, and `--display` requires concept results. Read [member filters](docs/ecl-support.md) for examples, limits and remaining type gaps.
