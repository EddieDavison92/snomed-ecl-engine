...
try {
    ...
    $results = :probe foreach ($query in $queries | Where-Object probe) {
        ...
        do {
            ...
            foreach ($entry in $page) {
                ...
                if ($entry.system -ne 'http://snomed.info/sct') { throw 'Unexpected code system.' }
        ...
        $canonical = if ($sorted.Count) { ($sorted -join "`n") + "`n" } else { '' }
    ...
    $report = [ordered]@{
        ...
        edition=$Version
        digestFormat='Numeric SCTID sort; UTF-8; LF separated; final LF for non-empty sets; no BOM.'
        results=@($results)
    }
